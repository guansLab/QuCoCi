...
from q_layers import QLayer22, QLayer4
import random
import numpy as np
import pandas as pd
...
def main(train_noise_model,test_noise_model,backend):
    ...
    processor = QiskitProcessor(use_real_qc=False,backend=backend,noise_model_name=backend.name(),initial_layout=[1,2,3,4]) 
    ...
    processor_simulation = QiskitProcessor(use_real_qc=False,noise_model= test_noise_model,noise_model_name=backend.name(),initial_layout=[1,2,3,4])
...
if __name__ == '__main__':
    ...
    path = 'calibration_data/ibmq_belem-2022-one-year-80gate.csv'
    df = pd.read_csv(path)
    df = pd.DataFrame(df)
    ...
    for i in range(0,len(df2)):
        ...
        acc,confidence = main(train_noise_model,test_property_model,backend)
