...
def main(train_noise_model,test_noise_model,backend,time):
    ...
    if train_flag:
        ...
        torch.save(best_model,'models/{}'.format(args.modelpath))
    else:
        model = torch.load('models/{}'.format(args.modelpath))
...
if __name__ == '__main__':
    ...
    for i in range(0,len(df2)):
        ...
        acc,confidence = main(train_noise_model,test_property_model,backend,df2['timestamp'].loc[i])
        df2['accuracy-map-test'].loc[i] = acc
